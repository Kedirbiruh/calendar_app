...
class EventController {
  Future<List<Map<String, dynamic>>> fetchData(DateTime date) async {
    try {
      final url = Uri.parse(
        "https://history.muffinlabs.com/date/${date.month}/${date.day}",
      );
      ...
      while (chosen.length < 5 && chosen.length < events.length) {
        ...
        if (!chosen.any(
          (event) => event["year"] == e["year"] && event["text"] == e["text"],
        )) {
          chosen.add(e);
        }
      }
      ...
      while (chosen.length < 5) {
        chosen.add({"year": "-", "text": "Kein weiteres Ereignis"});
      }
      ...
      return List.generate(
        5,
        (index) => {"year": "-", "text": "Daten konnten nicht geladen werden"},
      );
    }
  }
}
